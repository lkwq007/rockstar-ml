...
exception ArgParamNotConsistent of string
...
let eval prog=
  ...
  let rec eval_exp env=function
    ...
    | Call (f,lst) -> begin let (arg,b)=try Hashtbl.find func_lst f with Not_found -> raise (UndefinedFunction f)
                      ...
                      in let rec remove_arg env arg_=match arg_ with 
                        ...
                        | [] -> ()
                      in let ()=if (List.length arg)=(List.length lst) then add_arg env arg (extract_lst env lst) else raise (ArgParamNotConsistent f);
